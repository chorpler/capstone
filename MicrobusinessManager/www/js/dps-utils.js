...
(function factory(root) {
	...
	*/
	...
	// Started with:
	// https://gist.github.com/jonmaim/7b896cf5c8cfe932a3dd#file-csv2js-js
	// and
	// https://gist.github.com/iwek/7154578#file-csv-to-json-js
	// Fixes the issue with double quoted values
	function csv2json(csv, separator, header) {
		var lines=csv.trim().split("\n");
		var sep = separator || "\t";
		var result = [];
		var headers = header || lines[0].split(sep);
		var start = header ? 1 : 0;
		var hdr = [];
		for(var i=start; i<lines.length; i++) {
			var obj = {};

			var strRow = lines[i].trim(),
				queryIdx = 0,
				startValueIdx = 0,
				idx = 0;
	...
	function json2csv(jsonArray, separator) {
		var sep = separator || '\t';
		var eol = '\n';
		var csvOut = "";
		var header = "";
		var body = "";
		var i = 0;
		var arrHeader = Object.keys(jsonArray[0]);
		for(var str in arrHeader) {
			header += i > 0 ? sep : "";
			header += arrHeader[str];
			i++;
		}
		header += eol;
		...
		csvOut = header + body;
		Log.l("json2csv: original and final:");
		Log.l(jsonArray);
		Log.l(csvOut);
		return csvOut;
	}
